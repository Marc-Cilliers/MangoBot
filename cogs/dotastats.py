...
class MatchNotParsedError(UserError):
    def __init__(self, match_id, action=None):
        ...
        super().__init__(
            f"This match must be parsed before I can {self.action}.\nTry `/parse {match_id}` to request a parse."
        )
...
class StratzMatchNotParsedError(UserError):
    def __init__(self, match_id):
        super().__init__(
            f"It looks like match `{match_id}` hasn't been parsed by STRATZ. To have your matches parsed by STRATZ a bit faster, you can login to their site: <https://stratz.com>"
        )
...
class DotaMatch:
    ...
    async def convert(cls, inter: disnake.CmdInter, match_string: str):
        ...
        try:
            ...
            raise UserError(
                f"Couldn't find a match_id or a player when given '{match_string}'"
            )
...
async def get_stratz_match(match_id):
    if settings.stratz is None:
        raise UserError(
            "Stratz not configured properly. The bot owner has gotta put the stratz api key in the config file"
        )
...
def is_stratz_parsed(match):
    return (
        match.get("parsedDateTime")
        and match["players"][0].get("playbackData")
        and match["players"][0].get("playbackData").get("playerUpdatePositionEvents")
    )
...
class DotaStats(MangoCog):
    ...
    def __init__(self, bot):
        ...
        self.dota_game_strings = read_json(
    ...
    async def on_message(self, message: disnake.Message):
        logger.event("dotastats message received!", {"message": message.content})
    ...
    def sort_meta(self, json, num=10):
        ...
        sorted_json = sorted(
            ...
            key=lambda x: (
                ((x.get("pro_pick", 0) + x.get("pro_ban", 0)) / total_games)
    ...
    async def create_dota_gif(
        self, match, stratz_match, start_time, end_time, ms_per_second=100
    ):
        await self.dota_gif_lock.acquire()
        try:
            result = await drawdota.create_dota_gif(
                self.bot, match, stratz_match, start_time, end_time, ms_per_second
            )
        finally:
    ...
    async def get_firstblood_story(self, game, is_radiant):
        ...
        if fb_objective is None:
            return (
                ""  # No first blood this game, or it wasnt reported in objectives log
            )
        fb_log = None
        fb_killer = next(
            p
            for p in game["players"]
            if p["player_slot"] == fb_objective["player_slot"]
        )
        fb_log = next(
            (
                kill
                for kill in fb_killer["kills_log"]
                if kill["time"] == fb_objective["time"]
            ),
            None,
        )
        if fb_log is None:
        ...
        dotabase = self.bot.get_cog("Dotabase")
        fb_victim_id = next(
            h for h in self.hero_info if self.hero_info[h]["full_name"] == fb_log["key"]
        )
    ...
    async def get_lane_stories(self, game, is_radiant, use_icons=False):
        ...
        for laneid in lanes:
            story += "• {0[us]} {0[won_lost]} {1} lane vs {0[them]}\n".format(
                await self.get_lane_story(
                    game["players"], laneid, is_radiant, use_icons
                ),
                lanes[laneid],
            )
        roamers = [
            self.get_pretty_hero(p, use_icons)
            for p in game["players"]
            if p.get("is_roaming")
        ]
        if roamers:
    ...
    def set_match_footer(self, match, embed):
        ...
        embed.timestamp = datetime.datetime.fromtimestamp(
            match["start_time"], tz=datetime.timezone.utc
        )
    ...
    async def player_match_stats(self, steamid, match, inter, description_intro=None):
        ...
        if steamid:
            player = next(
                (p for p in match["players"] if p["account_id"] == steamid), None
            )
        if player is None:
        ...
        description = (
            f"{winstatus} a {lobby_type}**{game_mode}** match as {hero_name} in {duration}. "
        ...
        embed.set_author(
            ...
            icon_url=self.hero_info[player["hero_id"]]["icon"],
            url="https://www.opendota.com/players/{}".format(steamid),
        )
        ...
        match_image = disnake.File(
            await drawdota.create_match_image(match), "match.png"
        )
    ...
    async def lm(self, inter: disnake.CmdInter, matchfilter: MatchFilter = None):
        ...
        description_intro = (
            None if (not matchfilter) else matchfilterfixer(matchfilter.localize())
        )
    ...
    async def firstmatch(
        self, inter: disnake.CmdInter, matchfilter: MatchFilter = None
    ):
    ...
    async def postgame(self, inter: disnake.CmdInter, match_id):
        ...
        match_image = disnake.File(
            await drawdota.create_postgame_image(match, streamer_win),
            filename="matchimage.png",
        )

        if streamer_win:
            embed.set_thumbnail(
                url="https://cdn.glitch.com/fb7e55f2-3f82-4b8a-ba79-9a903f6a46b9%2Fpog.png?v=1614633965962"
            )
        else:
            embed.set_thumbnail(
                url="https://cdn.glitch.com/fb7e55f2-3f82-4b8a-ba79-9a903f6a46b9%2Fpepega.png?v=1614633959884"
            )
    ...
    async def print_match_stats(self, inter, match):
        ...
        embed.set_author(
            name="Match {}".format(match_id),
            url="https://www.opendota.com/matches/{}".format(match_id),
        )
        ...
        match_image = disnake.File(
            await drawdota.create_match_image(match), filename="matchimage.png"
        )
        embed.set_image(url=f"attachment://{match_image.filename}")
    ...
    async def match_story(
        ...
        match: DotaMatch,
        perspective: commands.option_enum(
            OrderedDict({"Radiant": "radiant", "Dire": "dire"})
        ) = "radiant",
    ):
        ...
        if steamid is not None:
            ...
            if steamid is not None and player_data is not None:
                ...
                perspective = "{2}({0}, {1})".format(
                    self.get_pretty_hero(player_data),
        ...
        story = (
            f"*Told from the perspective of {perspective}*\n"
        ...
        while (
            i < len(teamfights)
            and (len(story) + len(teamfights[i]) + len(story_end)) < 2000
        ):
            story += f"\n\n{teamfights[i]}"
            i += 1
    ...
    async def recent(self, inter: disnake.CmdInter, matchfilter: MatchFilter = None):
        ...
        matches_image = await drawdota.draw_matches_table(
            matches, self.dota_game_strings
        )
        matches_image = disnake.File(matches_image, "matches.png")
        embed.set_image(url=f"attachment://{matches_image.filename}")
        embed.set_footer(text="Try /matchids to get copy-pastable match ids")
    ...
    async def matchids(self, inter: disnake.CmdInter, matchfilter: MatchFilter = None):
        ...
        embed.set_footer(text="Try /recent to get more details about these matches")

        await inter.send(embed=embed)

    @commands.slash_command()
    async def meta(
        self, inter: disnake.CmdInter, count: commands.Range[int, 1, 120] = 10
    ):
        ...
        embed = disnake.Embed(description=description, color=self.embed_color)
        meta_table = disnake.File(
            await drawdota.draw_meta_table(sorted_json, json), "meta.png"
        )
    ...
    async def profile(self, inter: disnake.CmdInter, player: DotaPlayer = None):
        ...
        if gamesplayed > 0:
            winrate = "{:.2%}".format(
                len(
                    list(
                        filter(
                            lambda m: m.get("radiant_win", False)
                            == ((m.get("player_slot", 0) or 0) < 128),
                            matches,
                        )
                    )
                )
        ...
        for i in range(0, len(matches) - 1):
            delta = matches[i]["start_time"] - (
                matches[i + 1]["start_time"] + matches[i]["duration"]
            )
        ...
        overall_activity_delta = get_pretty_time(
            (int(statistics.mean(activity_delta)) // 60) * 60
        )
        if recent_count:
            recent_activity_delta = get_pretty_time(
                (int(statistics.mean(activity_delta[:recent_count])) // 60) * 60
            )
        else:
        ...
        embed.set_author(
            ...
            or f"https://www.opendota.com/players/{steam32}",
        )
        ...
        rank_icon = await drawdota.dota_rank_icon(
            playerinfo.get("rank_tier"), playerinfo.get("leaderboard_rank")
        )
    ...
    async def do_playerstats(
        self, inter: disnake.CmdInter, matchfilter: MatchFilter, do_downloaded=False
    ):
        matchfilter.add_projections(
            [
                "kills",
        ...
        if do_downloaded:
            ...
            while i < len(matches_info) and len(matches) < 20:
                if matches_info[i].get("version", None) is not None:
                    ...
                    player_match = next(
                        (p for p in match["players"] if p["account_id"] == steam32),
                        None,
                    )
        ...
        embed.set_footer(
            text='To see the filtering options for this command, try "/docs matchfilter"'
        )
        ...
        if matchfilter.has_value("included_account_id"):
            ...
            image = disnake.File(
                await drawdota.combine_image_halves(avatar1, avatar2), "profile.png"
            )
        ...
        def get_time_diff(match):
            ...
            if timediff > (29 * 60 * 60 * 24):  # if was over a month ago
                timediff -= timediff % (
                    60 * 60 * 24
                )  # only show up to days level of detail
            if timediff > (3 * 60 * 60 * 24):  # if was over a couple day ago
                timediff -= timediff % (
                    60 * 60
                )  # only show up to hours level of detail
        ...
        embed.set_author(name=author_name, icon_url=author_icon_url, url=matches_url)
        ...
        for match in player_matches:
            heroes[match["hero_id"]] = heroes.get(match["hero_id"], 0) + 1
        heroes = sorted(heroes.items(), key=lambda x: x[1], reverse=True)
        favorite_heroes = "".join(
            map(lambda h: self.hero_info[h[0]]["emoji"], heroes[0:3])
        )
        zeropercent = "0%"
        ...
        laning_postfix = ""
        parsed_count = len(
            list(filter(lambda m: m.get("version") is not None, player_matches))
        )
        ...
        class CoolStat:
            ...
            def should_show(self):
                return not (
                    self.value == self.ignore_value
                    or matchfilter.has_value(self.filter_key)
                )
        ...
        stat_sections = [
            {
                ...
                "stats": [
                    ...
                    CoolStat(
                        "Winrate",
                        percent(
                            lambda p: p.get("radiant_win")
                            == (p.get("player_slot") < 128)
                        ),
                        filter_key="win",
                    ),
                    ...
                    CoolStat(
                        "In a Party",
                        percent(
                            lambda p: p.get("party_size") > 1,
                            needs_key="party_size",
                            round_place="floor",
                        ),
                    ),
                    CoolStat(
                        "Ranked",
                        percent(lambda p: p["lobby_type"] == 7),
                        filter_key="lobby_type",
                    ),
                ],
            },
            {
                ...
                "stats": [
                    CoolStat(
                        self.get_emoji("attr_strength"),
                        percent(
                            lambda p: self.hero_info.get(p["hero_id"], {}).get("attr")
                            == "strength"
                        ),
                        separator=" ",
                    ),
                    CoolStat(
                        self.get_emoji("attr_agility"),
                        percent(
                            lambda p: self.hero_info.get(p["hero_id"], {}).get("attr")
                            == "agility"
                        ),
                    ...
                    CoolStat(
                        self.get_emoji("attr_intelligence"),
                        percent(
                            lambda p: self.hero_info.get(p["hero_id"], {}).get("attr")
                            == "intelligence"
                        ),
            ...
            },
            {
                ...
                "stats": [
                    CoolStat(
                        "Safe Lane",
                        percent(
                            lambda p: p.get("lane_role") == 1
                            and not p.get("is_roaming"),
                            needs_key="lane_role",
                        ),
                        ignore_value=zeropercent,
                    ),
                    CoolStat(
                        "Mid Lane",
                        percent(
                            lambda p: p.get("lane_role") == 2
                            and not p.get("is_roaming"),
                            needs_key="lane_role",
                        ),
                        ignore_value=zeropercent,
                    ...
                    CoolStat(
                        "Roaming",
                        percent(lambda p: p.get("is_roaming"), needs_key="is_roaming"),
                        ignore_value=zeropercent,
                    ),
                ],
            },
        ...
        if (
            do_downloaded
        ):  # if we've downloaded all of these matches, compute all the chat history stuff, and add the downloaded stats
            chat_wheel_counts = {}
            chat_wheel_total = 0
            ...
            for match in matches:
                player = next(
                    (p for p in match["players"] if p["account_id"] == steam32), None
                )
                    ...
                    for message in match_chat:
                        if message.get("player_slot", -1) == player["player_slot"]:
                            if message["type"] == "chat":
                                message_count += 1
                                if longest_message is None or len(
                                    longest_message
                                ) <= len(message["key"]):
                                ...
                                chat_wheel_counts[msg_id] = (
                                    chat_wheel_counts.get(msg_id, 0) + 1
                                )
                                chat_wheel_total += 1
            ...
            if chat_wheel_counts != {}:
                lines = []
                chat_wheel_counts = sorted(
                    chat_wheel_counts.items(), key=lambda m: m[1], reverse=True
                )
                for i in range(0, min(3, len(chat_wheel_counts))):
                    ...
                    icon = self.get_emoji(
                        "chat_wheel_sound"
                        if message.get("is_sound")
                        else "chat_wheel_text"
                    )
            ...
            stat_sections.extend(
                [
                    ...
                    {
                        "caption": "Chat Wheel",
                        "stats": [CoolStat(None, chat_wheel_text)],
                    },
                    {
                        "caption": "All Chat",
                        "inline": False,
                        "stats": [
                            ...
                            CoolStat(
                                longest_message_heading, longest_message, bold=False
                            ),
                        ],
                    },
                ]
        ...
        for category in stat_sections:
            if category.get("filter_key") and matchfilter.has_value(
                category.get("filter_key")
            ):
                continue  # skip this category if its already filtered out by the matchfilter
            value = "\n".join(
                map(
                    lambda s: s.render(),
                    filter(lambda s: s.should_show(), category.get("stats")),
                )
            )
            if value == "":
            ...
            embed.add_field(
                ...
                value=value,
                inline=category.get("inline", True),
            )
    ...
    async def dotagif(
        ...
        image = disnake.File(
            await self.create_dota_gif(match, stratz_match, start, end, ms_per_second),
            "map.gif",
        )
    ...
    async def match_laning(self, inter: disnake.CmdInter, match: DotaMatch):
        ...
        embed = disnake.Embed(
            description=await self.get_lane_stories(match, perspective, True)
        )
        ...
        image = disnake.File(
            await self.create_dota_gif(match, stratz_match, -89, 600, 100), "map.gif"
        )
    ...
    async def parse(self, inter: disnake.CmdInter, match_id: int = None):
        ...
        try:
            data = await httpgetter.post(
                f"https://api.opendota.com/api/request/{match_id}",
                errors=opendota_html_errors,
            )
        except HttpError as e:
            if e.code == 400:
        ...
        if data.get("status") == "failed" or data.get("err") is not None:
            await inter.send(
                f"❌ There was an error requesting the parse for match {match_id}"
            )
            return
        ...
        await inter.send(
            f"❌ Parsing of match {match_id} timed out. Try again later or on the opendota site.",
            delete_after=10,
        )

    @commands.command()
    async def whoishere(
        self, inter: disnake.CmdInter, users: str = None, show_ranks: bool = False
    ):
        ...
        if inter.author.voice and inter.author.voice.channel:
            ...
            if audioplayer is None or audioplayer.voice_channel is None:
                if len(additional_user_ids) == 0:
                    raise UserError(
                        "One of us needs to be in a voice channel for that to work"
                    )
            else:
        ...
        for user_id in members:
            ...
            if userinfo.steam is None:
                ...
                links.append(
                    f"[{player_info['profile']['personaname']}](https://www.opendota.com/players/{userinfo.steam})"
                )
    ...
    async def rolesgraph(self, inter: disnake.CmdInter, player: DotaPlayer = None):
        ...
        role_totals_modifiers = list(
            map(lambda x: role_totals_avg / (x or 1), role_totals)
        )
        ...
        embed.set_author(
            ...
            icon_url=playerinfo["profile"]["avatar"] or default_steam_icon,
            url=playerinfo["profile"]["profileurl"]
            or f"https://www.opendota.com/players/{player.steam_id}",
        )

        image = disnake.File(
            drawdota.draw_polygraph(role_scores, roles), "rolesgraph.png"
        )
        embed.set_image(url=f"attachment://{image.filename}")
        await inter.send(embed=embed, file=image)
    ...
    async def match_skillbuild(self, inter: disnake.CmdInter, match: DotaMatch):
        ...
        image = disnake.File(
            await drawdota.draw_match_ability_upgrades(match), "upgrades.png"
        )
        embed.set_image(url=f"attachment://{image.filename}")
    ...
    async def match_graph(
        self,
        inter: disnake.CmdInter,
        match: DotaMatch,
        graphtype: commands.option_enum(OrderedDict(graphtypes)) = "teamdiff",
    ):
        ...
        embed.description = next(
            key for key, value in graphtypes.items() if value == graphtype
        )
        ...
        if graphtype == "teamdiff":
            ...
            for player in match["players"]:
                colors.append(
                    playercolors[str(player["player_slot"])]
                    if str(player["player_slot"])
                    else "#FF0000"
                )
                lines.append(player["gold_t"])
                labels.append(
                    self.hero_info[player["hero_id"]]["name"]
                    if player["hero_id"] in self.hero_info
                    else "Unknown"
                )
        else:
    ...
    async def inventory(self, inter: disnake.CmdInter, matchfilter: MatchFilter = None):
        ...
        slot_item_counts = list(
            map(
                lambda item_counts: sorted(
        ...
        for slot in range(len(slot_item_counts)):
            for item, count in slot_item_counts[slot]:
                if (item not in items_best_slots) or (
                    items_best_slots[item][1] < count
                ):
        ...
        for item, slot_count in items_best_slots.items():
            ...
            for i in range(len(slot_item_counts)):
                if i != slot:
                    slot_item_counts[i] = list(
                        filter(
                            lambda item_count: item_count[0] != item,
                            slot_item_counts[i],
                        )
                    )
        ...
        embed.url = (
            f"https://www.opendota.com/players/{matchfilter.player.steam_id}/matches"
        )
        query_args = matchfilter.to_query_args()
        if query_args:
            embed.url += "?" + query_args
        ...
        embed.description = matchfilter.localize()
        embed.set_footer(
            text="The 3 most common items for each inventory slot. (At the end of the game)"
        )

        image = disnake.File(
            await drawdota.draw_item_slots(slot_item_counts), "items.png"
        )
        embed.set_image(url=f"attachment://{image.filename}")
    ...
    async def percent(
        self,
        inter: disnake.CmdInter,
        filter1: MatchFilter = None,
        filter2: MatchFilter = None,
    ):
        ...
        description += (
            "**Out of** " + filter1 + ", **what percent of them were** " + filter2 + "?"
        )
